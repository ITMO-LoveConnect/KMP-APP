...
import ru.connect.core.cache.CachePolicy.Type.ALWAYS
import ru.connect.core.cache.CachePolicy.Type.CLEAR
import ru.connect.core.cache.CachePolicy.Type.EXPIRES
import ru.connect.core.cache.CachePolicy.Type.LOCAL
import ru.connect.core.cache.CachePolicy.Type.NEVER
import ru.connect.core.cache.CachePolicy.Type.REFRESH
    ...
    suspend fun fetch(request: Request, cachePolicy: CachePolicy, useLock: Boolean = false): Response? =
        withOptionalLock(useLock) {
            when (cachePolicy.type) {
                ...
                EXPIRES -> {
                    localDataSource.get(request)?.let {
                        if ((it.createdAt + cachePolicy.expires) > Clock.System.now().toEpochMilliseconds()) {
                            it.value
                        } else {
...
interface LocalDataSource<in Key : Any, Result> {
    ...
    fun observeKeys(): Flow<Set<@UnsafeVariance Key>>
    suspend fun getKeys(): Set<@UnsafeVariance Key>
}
